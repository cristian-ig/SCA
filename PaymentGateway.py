...
from Utils import saveKey, loadKey, get_money_from_card, withdraw_money_from_card
...
try:
    ...
    pkcs1_15.new(key).verify(SHA256.new(bin(int.from_bytes(decrypted_data['USigM'], byteorder='big')).encode("UTF-8")),
                             (SigM))
    verification = True
...
if verification:
    ...
    print("sa incepem")
    print(PM['Amount'])
    print(get_money_from_card(PM['CardNumber']))
    if int(PM['Amount']) <= get_money_from_card(PM['CardNumber']):
        withdraw_money_from_card(PM['CardNumber'], PM['Amount'])
        resp = "True"

    uSigPg = {"Response": resp, "SID": PM['SID'], "Amount": PM['Amount']}
    key = RSA.import_key(loadKey('pgSK'))
    response_for_merchant = {"Response": resp, "SID": PM['SID'], "SigPG": pkcs1_15.new(key).sign(
        SHA256.new(bin(int.from_bytes(pickle.dumps(uSigPg), byteorder='big')).encode("UTF-8")))}
